...
final case class SignalTypeMismatchException(message: String) extends RuntimeException(message)
final case class UnknownSignalWidthException(path: String)
    extends RuntimeException(s"Signal '$path' has unknown width")
final case class FailedExpectException(message: String) extends RuntimeException(message)
